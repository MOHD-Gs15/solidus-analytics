...
/**
 * Premium fraud scanner over Core's transaction ledger.
 *
 * <p>Read path (2.1.4): every check runs against the SAME connection source
 * as the engine collectors. When Core runs its 2.2.x MySQL network mode the
 * {@link CoreLedgerAccess} bridge (set by the engine) executes each query on
 * Core's own live connection - backend-agnostic, no local SQLite files.
 * In SQLite mode (bridge null) the checks fall back to short-lived read-only
 * connections to economy.db exactly like before. Either way a failed scan
 * degrades to an empty alert list (fail-open, premium-only path).</p>
 */
public class FraudDetector {
    ...
    /**
     * Runs one read against Core's ledger: through the CoreLedgerAccess
     * bridge when Core is in MySQL network mode, otherwise on a short-lived
     * read-only connection to the economy.db file. The work object closes
     ...
     */
    private <T> T withLedger(LedgerWork<T> work) throws SQLException {
    ...
    private List<FraudAlert> checkRapidWealthGain() {
        ...
        return new ArrayList<FraudAlert>();
    }
